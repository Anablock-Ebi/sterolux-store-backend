...
export default async function seedDemoData({ container }: ExecArgs) {
  ...
  try {
    ...
    // Check if data already exists - comprehensive check
    const [store] = await storeModuleService.listStores();
    const existingProducts = await container
      .resolve(ModuleRegistrationName.PRODUCT)
      .listProducts();

    // More robust currency check - specifically look for EUR and USD
    const storeHasCurrencies =
    ...
    // Also check if price preferences exist directly
    let hasPricePreferences = false;
    try {
      ...
      hasPricePreferences = pricePreferences.length > 0;
    } catch (error) {
      logger.info("Could not check price preferences, continuing...");
    }

    // Check for regions to see if basic setup is done
    const regionModuleService = container.resolve(
      ModuleRegistrationName.REGION
    );
    const existingRegions = await regionModuleService.listRegions();

    // If we have products, currencies, and regions, assume full seeding is done
    if (
      existingProducts.length > 0 &&
      (storeHasCurrencies || hasPricePreferences) &&
      existingRegions.length > 0
    ) {
      logger.info(
        "Store data appears to be fully seeded already, skipping seed..."
      );
      return;
    } else if (existingProducts.length > 0) {
      logger.info(
        "Products exist but other data may be incomplete, continuing with conditional seeding..."
      );
    }
    ...
    // Check if store already has currencies configured to avoid duplicate price preferences
    const shouldSkipCurrencySetup = storeHasCurrencies || hasPricePreferences;
    logger.info(
      `Store currencies check: ${
        storeHasCurrencies ? "Has currencies" : "No currencies"
      }`
    );
    logger.info(
      `Price preferences check: ${
        hasPricePreferences ? "Has preferences" : "No preferences"
      }`
    );
    logger.info(
      `Store supported currencies: ${JSON.stringify(
        store.supported_currencies
      )}`
    );

    if (!shouldSkipCurrencySetup) {
      try {
        ...
        logger.info(
          "Successfully updated store with currencies and sales channel"
        );
      } catch (error) {
        if (
          error.message &&
          (error.message.includes("Price preference") ||
            error.message.includes("already exists"))
        ) {
          logger.info(
            "Store currencies already exist (caught duplicate error), skipping currency setup..."
          );
        } else {
          logger.error("Unexpected error updating store:", error);
          throw error;
        }
      }
    } else {
      // Only update sales channel if currencies already exist
      if (store.default_sales_channel_id !== defaultSalesChannel[0].id) {
        try {
          await updateStoresWorkflow(container).run({
            input: {
              selector: { id: store.id },
          ...
          logger.info("Successfully updated store with sales channel only");
        } catch (error) {
          logger.info(
            `Error updating sales channel, may already be set: ${error.message}`
          );
        }
      } else {
        logger.info("Store already has correct sales channel, skipping update");
      }
    }
    logger.info("Seeding region data...");
    const europeRegions = await regionModuleService.listRegions({
      name: "Europe",
    });

    let region;
    if (europeRegions.length > 0) {
      region = europeRegions[0];
    ...
    if (existingStockLocations.length > 0) {
      ...
      logger.info(
        "European Warehouse already exists, skipping stock location creation..."
      );
    } else {
    ...
    // Create link between stock location and fulfillment provider (with error handling for duplicates)
    try {
      await link.create({
      ...
      logger.info(
        "Stock location to fulfillment provider link already exists, skipping..."
      );
    }

    logger.info("Seeding fulfillment data...");

    // Check for existing shipping profiles
    ...
    if (existingShippingProfiles.length > 0) {
      ...
      logger.info(
        "Default shipping profile already exists, skipping creation..."
      );
    } else {
    ...
    if (existingFulfillmentSets.length > 0) {
      ...
      logger.info(
        "European Warehouse delivery fulfillment set already exists, skipping creation..."
      );
    } else {
    ...
    // Create link between stock location and fulfillment set (with error handling for duplicates)
    try {
      await link.create({
        [Modules.STOCK_LOCATION]: {
          stock_location_id: stockLocation.id,
      ...
      logger.info(
        "Stock location to fulfillment set link already exists, skipping..."
      );
    }
    ...
    if (existingShippingOptions.length === 0) {
      ...
      logger.info(
        "Shipping options already exist, skipping shipping options creation..."
      );
    }
    logger.info("Finished seeding fulfillment data.");

    // Link sales channels to stock location (with error handling for duplicates)
    try {
    ...
    if (existingApiKeys.length > 0) {
      ...
      logger.info(
        "Webshop API key already exists, skipping API key creation..."
      );
    } else {
      ...
      try {
        ...
        logger.info(
          "Sales channel to API key link already exists, skipping..."
        );
      }
    }
    ...
    const existingCollections =
      await productModuleService.listProductCollections({ handle: "featured" });

    let collection;
    if (existingCollections.length > 0) {
      collection = existingCollections[0];
      logger.info(
        "Featured collection already exists, skipping collection creation..."
      );
    } else {
      const {
    ...
    if (existingCategories.length > 0) {
      ...
      const { result: createdCategoryResult } =
      ...
      categoryResult = createdCategoryResult;
    }
    ...
    if (process.env.ADMIN_EMAIL) {
      ...
      try {
        ...
        if (existingInvites.length === 0) {
          ...
          logger.info(
            `Complete setup at: [your-admin-url]/invite?token=${invite.token}`
          );
        } else {
          logger.info(
            `Admin invite already exists for ${process.env.ADMIN_EMAIL}`
          );
        }
      } catch (error) {
